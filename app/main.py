...
def load_models():
    ...
    try:
        ...
        if not os.path.exists(PRECIPITATION_MODEL_PATH):
            raise ModelLoadError(f"Precipitation model file not found: {PRECIPITATION_MODEL_PATH}")
        if not os.path.exists(PRECIPITATION_INFO_PATH):
            raise ModelLoadError(f"Precipitation model info file not found: {PRECIPITATION_INFO_PATH}")
            ...
            if feature not in daily_data:
                ...
                if value is None:
                    ...
                    if 'temperature' in feature and (value < -50 or value > 60):
                        logger.warning(f"Suspicious temperature value: {value} for {feature}")
...
async def health_check():
    ...
    all_healthy = rain_model_status and precipitation_model_status and weather_api_status
    ...
    return {
        ...
        "checks": {
            "models": {
                ...
                "precipitation_model": {
                    "loaded": precipitation_model_status
                }
            },
            "external_services": {
                "weather_api": {
...
def check_rate_limit():
    ...
    request_timestamps = [t for t in request_timestamps if current_time - t < RATE_LIMIT_WINDOW]
...
def _predict_rain_sync(date: str, prediction_date: datetime) -> Dict[str, Any]:
    ...
    return {
        ...
        "prediction": {
            ...
            "confidence": float(prediction_proba) if will_rain else float(1 - prediction_proba)
        },
        "metadata": {
            "model_version": "2.0.0",
            "threshold": rain_threshold
...
async def predict_precipitation_fall(
    ...
    try:
        ...
        logger.error(f"Unexpected error in precipitation prediction: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="An unexpected error occurred during prediction"
        )

def _predict_precipitation_sync(date: str, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
...
async def startup_event():
    ...
    logger.info(f"Models loaded: Rain={rain_model is not None}, Precipitation={precipitation_model is not None}")
